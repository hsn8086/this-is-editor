...
describe('services', () => {
  ...
  describe('Cache', () => {
    ...
    it('should expire keys when calling keys()', async () => {
      cache.set('key1', 'value1', 10) // 10ms TTL
      cache.set('key2', 'value2', 60_000) // 60s TTL
    ...
    it('should expire keys when calling deleteByPrefix()', async () => {
      ...
      cache.set('prefix:key2', 'value2', 60_000) // 60s TTL
  ...
  describe('TaskService', () => {
    ...
    it('should run task', async () => {
      ...
      const result = await taskService.runTask(1, 256, 1)
      expect(result).toEqual(mockResult)
      expect(getMockApi().run_task).toHaveBeenCalledWith(1, 256, 1)
    })
